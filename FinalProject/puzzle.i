...
extern const unsigned short wormMap[1024];
# 13 "puzzle.c" 2
# 1 "bumbleBee.h" 1
...
extern const unsigned short bumbleBeeMap[1024];
# 14 "puzzle.c" 2
# 1 "prayingMantis.h" 1
...
extern const unsigned short prayingMantisMap[1024];
# 15 "puzzle.c" 2
# 1 "violinBeetle.h" 1
...
extern const unsigned short violinBeetleMap[1024];
# 16 "puzzle.c" 2
# 1 "leafBug.h" 1
...
extern const unsigned short leafBugMap[1024];
# 17 "puzzle.c" 2
# 1 "grasshopper.h" 1
...
extern const unsigned short grasshopperMap[1024];
# 18 "puzzle.c" 2
# 1 "stagBeetle.h" 1
...
extern const unsigned short stagBeetleMap[1024];
# 19 "puzzle.c" 2
# 1 "shieldBug.h" 1
...
extern const unsigned short shieldBugMap[1024];
# 20 "puzzle.c" 2
# 1 "dragonHeadedCatepillar.h" 1
...
extern const unsigned short dragonHeadedCatepillarMap[1024];
# 21 "puzzle.c" 2
# 1 "acornWeevil.h" 1
...
extern const unsigned short acornWeevilMap[1024];
# 22 "puzzle.c" 2
# 1 "rosyMapleMoth.h" 1
...
extern const unsigned short rosyMapleMothMap[1024];
# 23 "puzzle.c" 2
# 1 "interrupts.h" 1
...
void initializePuzzle(){
    (*(volatile unsigned short *)0x4000000) = ((0) & 7) | (1 << 12) | (1 << (8 + (0 % 4)));
    switch (currentBug){
        case (BLUEMORPHO):
            DMANow(3, blueMorphoMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (WORM):
            DMANow(3, wormMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (BUMBLEBEE):
            DMANow(3, bumbleBeeMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (MANTIS):
            DMANow(3, prayingMantisMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (VIOLIN):
            DMANow(3, violinBeetleMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (LEAFBUG):
            DMANow(3, leafBugMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (GRASSHOPPER):
            DMANow(3, grasshopperMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (STAG):
            DMANow(3, stagBeetleMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (SHIELD):
            DMANow(3, shieldBugMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (DRAGON):
            DMANow(3, dragonHeadedCatepillarMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (ACORN):
            DMANow(3, acornWeevilMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
        case (MOTH):
            DMANow(3, rosyMapleMothMap, &((screenblock *)0x6000000)[31], (2048)/2);
            break;
    }
...
void updatePuzzle(){
    if ((!(~(oldButtons) & ((1<<9))) && (~(buttons) & ((1<<9)))) && (!(~(oldButtons) & ((1<<8))) && (~(buttons) & ((1<<8))))){
        ...
        for (int i = 0; i < bugs[currentBug].rarity; i++){
            if (bugs[currentBug].solution[i] == 0){
                ((screenblock *)0x6000000)[31].tilemap[(((bugs[currentBug].startY/8) + (i / (bugs[currentBug].puzzleWidth/8))) * (32) + ((bugs[currentBug].startX/8) + (i % (bugs[currentBug].puzzleWidth/8))))] = ((0) * (32) + (3));
            }
        }
    }
    if ((!(~(oldButtons) & ((1<<5))) && (~(buttons) & ((1<<5)))) && cursor.x > bugs[currentBug].startX){
        cursor.x -= cursor.width;
    }
    if ((!(~(oldButtons) & ((1<<4))) && (~(buttons) & ((1<<4)))) && cursor.x < (bugs[currentBug].puzzleWidth + bugs[currentBug].startX - 8)){
        cursor.x += cursor.width;
    }
    if ((!(~(oldButtons) & ((1<<6))) && (~(buttons) & ((1<<6)))) && cursor.y > bugs[currentBug].startY){
        cursor.y -= cursor.height;
    }
    if ((!(~(oldButtons) & ((1<<7))) && (~(buttons) & ((1<<7)))) && cursor.y < (bugs[currentBug].puzzleWidth + bugs[currentBug].startY - 8)){
        cursor.y += cursor.height;
    }
    ...
    if ((!(~(oldButtons) & ((1<<0))) && (~(buttons) & ((1<<0)))) && !bugs[currentBug].found){
        switch (brush.x){
            case (192):
                ((screenblock *)0x6000000)[31].tilemap[((cursor.y/8) * (32) + (cursor.x/8))] = ((0) * (32) + (1));
                playerPuzzle[(((cursor.y - bugs[currentBug].startY)/8) * (bugs[currentBug].puzzleWidth/8) + ((cursor.x - bugs[currentBug].startX)/8))] = 0;
                break;
            case (200):
                ((screenblock *)0x6000000)[31].tilemap[((cursor.y/8) * (32) + (cursor.x/8))] = ((0) * (32) + (3));
                playerPuzzle[(((cursor.y - bugs[currentBug].startY)/8) * (bugs[currentBug].puzzleWidth/8) + ((cursor.x - bugs[currentBug].startX)/8))] = 0;
                break;
            case (208):
                ((screenblock *)0x6000000)[31].tilemap[((cursor.y/8) * (32) + (cursor.x/8))] = ((0) * (32) + (2));
                playerPuzzle[(((cursor.y - bugs[currentBug].startY)/8) * (bugs[currentBug].puzzleWidth/8) + ((cursor.x - bugs[currentBug].startX)/8))] = 1;
                break;
        }
    }

    shadowOAM[0].attr0 = (0<<14) | ((cursor.y) & 0xFF) | (0<<8);
    shadowOAM[0].attr1 = (0<<14) | ((cursor.x) & 0x1FF);
    shadowOAM[0].attr2 = (((0) & 0xF) << 12) | (((0) * 32 + (0)) & 0x3FF);

    shadowOAM[1].attr0 = (0<<14) | ((brush.y) & 0xFF) | (0<<8);
    shadowOAM[1].attr1 = (0<<14) | ((brush.x) & 0x1FF);
    shadowOAM[1].attr2 = (((0) & 0xF) << 12) | (((0) * 32 + (0)) & 0x3FF);
    ...
    for (int i = 0; i < bugs[currentBug].rarity; i++){
        if (playerPuzzle[i] != bugs[currentBug].solution[i]){
            ...
            free(playerPuzzle);
            ((unsigned short *)0x5000000)[6] = (((31) & 31) | ((31) & 31) << 5 | ((31) & 31) << 10);
            ((unsigned short *)0x5000000)[3] = (((31) & 31) | ((31) & 31) << 5 | ((31) & 31) << 10);
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (19))] = 111;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (20))] = 113;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (21))] = 100;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (22))] = 114;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (23))] = 114;

            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (25))] = 114;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (26))] = 115;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (27))] = 96;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (28))] = 113;
            ((screenblock *)0x6000000)[31].tilemap[((19) * (32) + (29))] = 115;
            playSolveEffect();
        }
    }

    if ((!(~(oldButtons) & ((1<<3))) && (~(buttons) & ((1<<3)))) && bugs[currentBug].found){
        goToRoom();
        ((unsigned short *)0x5000000)[6] = (((0) & 31) | ((0) & 31) << 5 | ((0) & 31) << 10);
        ((unsigned short *)0x5000000)[3] = (((31) & 31) | ((15) & 31) << 5 | ((15) & 31) << 10);
    }
}
